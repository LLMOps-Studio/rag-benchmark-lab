...
st.set_page_config(page_title="RAG Benchmark Lab", page_icon="🧪", layout="wide")
...
def load_golden_dataset():
    ...
    if os.path.exists(DATASET_PATH):
        ...
        unique_contexts = list({item["context"] for item in data})
        ...
        fallback_text = (
            "Modern vector databases, such as ChromaDB, utilize HNSW graphs..."
        )
...
if not available_models:
    st.sidebar.warning(
        f"No models found at {ollama_client.host}. Please pull one below."
    )
...
if st.sidebar.button("Pull from Registry") and new_model_name:
...
chunk_sizes_raw = st.sidebar.text_input(
    "Chunk Sizes (comma-separated)", value="150, 300"
)
chunk_overlaps_raw = st.sidebar.text_input(
    "Chunk Overlaps (comma-separated)", value="15, 30"
)
...
with col_text:
    ...
    raw_document = st.text_area(
        "Paste context source text here:", value=default_kb_text, height=300
    )

with col_queries:
    st.subheader("❓ Golden Query Evaluation Dataset")
    queries_raw = st.text_area(
        "Enter evaluation questions (One per line):", value=default_queries, height=300
    )
...
if st.button("🚀 Execute Optimization Benchmark", type="primary"):
    if not selected_models:
        ...
        st.info(
            f"Grid search active. Testing {len(selected_models) * len(chunk_sizes) * len(chunk_overlaps)} configurations..."
        )

        runner = RAGBenchmarkRunner(experiment_name="RAG_Optimization_Lab")

        with st.spinner(
            "Processing embeddings, evaluating LLM-as-a-Judge, and flushing VRAM dynamically..."
        ):
            ...
            st.success(
                "Optimization run completed successfully! Metrics synchronized to MLflow."
            )
            ...
            st.dataframe(
                df_results.sort_values(
                    by=["avg_faithfulness", "avg_latency"], ascending=[False, True]
                ),
            ...
            with col_chart1:
                ...
                st.bar_chart(
                    df_results, x="run_name", y=["avg_faithfulness", "avg_relevance"]
                )
