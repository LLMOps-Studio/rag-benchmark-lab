...
class RAGPipeline:
    ...
    def __init__(self, collection_name: str = "rag_benchmark"):
        ...
        # Ensure a clean slate for each independent pipeline configuration
        # run -- fails harmlessly if the collection doesn't exist yet.
        try:
    ...
    def ingest_documents(self, raw_text: str, chunk_size: int, chunk_overlap: int):
        ...
        self.chroma.add_documents(
            collection_name=self.collection_name, documents=chunks
        )
        return len(chunks)

    def answer_query(
        self, query: str, model_name: str, n_results: int = 2
    ) -> dict[str, Any]:
        ...
        return {"query": query, "context": context, "answer": response}
