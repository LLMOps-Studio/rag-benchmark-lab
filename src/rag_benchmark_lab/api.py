...
def run_batch_evaluation(request: BatchEvalRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        for item in request.dataset:
            ...
            # Test B: Neuro-Symbolic Token Match (e.g., P_8_V_9)
            elif re.search(r'P_[0-9]_V_[0-9]', expected):
                # Ensure the exact token format is present in the LLM's raw generation
        ...
        format_penalty_score = (format_violations / total_items) * 100 if total_items > 0 else 0.0
