...
class RAGBenchmarkRunner:
    ...
    def run_grid_benchmark(self, raw_text: str, test_queries: List[str], chunk_sizes: List[int], chunk_overlaps: List[int], models: List[str]) -> List[Dict[str, Any]]:
        results_summary = []
        ...
        for model_name in models:
            try:
                    ...
                    for chunk_overlap in chunk_overlaps:
                        ...
                        for query in test_queries:
                            with timing_context() as timer:
                                ...
                                output = pipeline.answer_query(query=query, model_name=model_name)
                            ...
                            faithfulness = self.evaluator.evaluate_faithfulness(context=output["context"], response=output["answer"])
                            relevance = self.evaluator.evaluate_relevance(query=output["query"], response=output["answer"])
                ...
                print(f"❌ [Hardware Limit Reached] Pipeline failed for {model_name}: {e}")
                
            finally:
                # KRİTİK NOKTA: İşlem başarılı olsa da, çökse de bu modelin VRAM'ini ZORLA BOŞALT
                print(f"[HARDWARE OPTIMIZER] Releasing VRAM layers for model: {model_name}")
